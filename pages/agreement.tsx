...
이 약관은 (주)에이블리코퍼레이션 회사(전자거래 사업자)가 운영하는 에이블리 사이버 몰(이하 "몰"이라 한다)에서 제공하는 인터넷 관련 서비스(이하 "서비스"라 한다)를 이용함에 있어 사이버몰과 이용자의 권리·의무 및 책임사항을 규정함을 목적으로 합니다. ※ 「PC통신등을 이용하는 전자거래에 대해서도 그 성질에 반하지 않는 한 이 약관을 준용합니다」

제 2조(정의)

1.  “몰”이란 (주)에이블리코퍼레이션 회사가 재화 또는 용역을 이용자에게 제공하기 위하여 컴퓨터 등 정보통신설비를 이용하여 재화 또는 용역을 거래할 수 있도록 설정한 가상의 영업장을 말하며, 아울러 사이버몰을 운영하는 사업자의 의미로도 사용합니다.
...
1.  회사에서 상품을 구매할 경우 각 상품별로 설정되어 누적되는 점수로써, 회사가 회원에게 제공하는 회원보상체계의 일종입니다.

2.  회원은 적립금을 제3자와 유상으로 거래하거나 현금으로 전환할 수 없습니다.
...
5.  회원탈퇴 또는 회원자격상실 시 잔액 여부와 상관없이 회원의 적립금은 소멸되며 타인에게 양도할 수 없습니다.
